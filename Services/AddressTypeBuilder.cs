...
namespace TenantAPI.Services {
    ...
    public class AddressTypeBuilder : IAddressTypeBuilder {
        ...
        public async Task<BuilderResponse> GetAddressType_ById(int id) {
            var query = Search(_entities.AddressTypes, x => x.Id == id).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; AddressType with id = '{id}' doesn't exist." }); 
            }
        }
        ...
        public async Task<BuilderResponse> UpdateAddressType(AddressTypeModel model) {
            ...
            var query = Search(_entities.AddressTypes, x =>  x.Id == model.Id);
            if (!query.Any()) {
                return new BuilderResponse { ValidationMessage = "Record Not Found; " + string.Format("AddressType with _id = '{0}' doesn't exist.",model.Id)}; 
            }
        ...
        public async Task<BuilderResponse> DeleteAddressType(int id) {

            var query = Search(_entities.AddressTypes, x => x.Id == id);
            if (!query.Any()) {
                return new BuilderResponse { ValidationMessage = "Record Not Found; " + string.Format("AddressType with _id = '{0}' doesn't exist.",id)}; 
            }
